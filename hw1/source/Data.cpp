...
{
    if (a.size() != b.size()) { std::cerr << "a+b: a.size() != b.size(), return originally left point." << std::endl; return a; }
...
double dataset_t::delta(const dataset_t& other, const std::function<double(int,int)>& loss)
{
	return score(*this, other, loss);
}
...
double score(const dataset_t& a, const dataset_t& b, const std::function<double(int, int)>& loss)
{
	...
	for (std::size_t i : std::views::iota((std::size_t)0, a.size()))
		ret += loss(a[i].label, b[i].label);
	return ret;
}
// unit test
#ifdef DATATEST
