...
public:
    ...
    double epsilon;
    OPTICS_t() = default;
    OPTICS_t(std::size_t minPts_, double epsilon_): minPts{minPts_}, epsilon{epsilon_} {};
...
template<typename T, typename U>
point_t<U> OPTICS_t<T, U>::fit(const std::vector<point_t<T>>& X)
{
    point_t<U> labels;
...
template<typename T, typename U>
point_t<U> OPTICS_t<T, U>::fit(const dataset_t<T, U>& dataset)
{
